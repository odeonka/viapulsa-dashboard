...
export default {
  ...
  methods: {
    ...
    displayTrx: function(trx){
      ...
      if(trx.trxDocID != null){
        db.collection('sandbox').doc(trx.TrxDocID)
        .update({
          terbaca : true
        })
      } else {
        db.collection('sandbox').where('trx_id', '==', trx.trx_id).get().then(function(docs){
          docs.docs.forEach((data) => {
            db.collection('sandbox').doc(data.id)
            .update({
              TrxDocID: data.id,
              terbaca : true
            })
    ...
    updateNomorServer(){
      ...
      db.collection('sandbox').doc(this.activetrx.TrxDocID)
      .update({
      ...
      storageRef.on(`state_changed`,snapshot=>{
        ...
        ()=>{this.uploadValue=100;
          storageRef.snapshot.ref.getDownloadURL().then((url)=>{
            ...
            var trxDataRef = db.collection('sandbox').doc(trxDocID);
    ...
    setAsSelesai(){
      ...
      db.collection('sandbox').doc(vm.activetrx.TrxDocID)
      .update({
        indexStatus : 4,
        completea: true
      }).then(function(){
  ...
  firestore () {
    return {
      transaksi: db.collection('sandbox').orderBy('time_complete', 'desc').orderBy('created', 'desc').limit(2000),
      countunread: db.collection('sandbox').where('terbaca', '==', false),
    }
  }
}
</script>