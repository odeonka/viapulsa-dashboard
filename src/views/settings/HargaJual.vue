<template>
  <div class="min-vw-100 min-vh-100 position-fixed" style="top:0;left:0;background:rgba(0,0,0,0.7);z-index:99999">
    <div class="bg-white w-75 overflow-auto mx-auto mt-5 p-3" style='border-radius:8px;height:85vh'>
      <div class="container">
        <div class="row">
          <div class="col-8">
              ...
              <b-tab title="Telkomsel" active>
                <table class="table table-responsive-sm table-hover table-outline mb-0">
                  <thead class="thead-light">
                    ...
                    <tbody>
                      <tr v-for="(harga, index) in hargatsel" :key='index'>
              ...
              <b-tab title="XL">
                <table class="table table-responsive-sm table-hover table-outline mb-0">
                  <thead class="thead-light">
                    ...
                    <tbody>
                      <tr v-for="(harga, index) in hargaxl" :key='index'>
                          <td class="text-center align-middle"><div class="avatar">
                              <img class="img-avatar" src="@/assets/img/provider/xlaxis-logo.png" width="60px">
                          </div></td>
                          <td class="align-middle">
                              {{formatNominal(harga.nominal)}}K
                          </td>
              ...
              <b-tab title="Indosat">
                <table class="table table-responsive-sm table-hover table-outline mb-0">
                  <thead class="thead-light">
                    ...
                    <tbody>
                      <tr v-for="(harga, index) in hargaindosat" :key='index'>
                          <td class="text-center align-middle"><div class="avatar">
              ...
              <b-tab title="Three">
                <table class="table table-responsive-sm table-hover table-outline mb-0">
                  <thead class="thead-light">
                    ...
                    <tbody>
                      <tr v-for="(harga, index) in hargathree" :key='index'>
                          <td class="text-center align-middle"><div class="avatar">
              ...
              <b-tab title="Axis">
                <table class="table table-responsive-sm table-hover table-outline mb-0">
                  <thead class="thead-light">
                    ...
                    <tbody>
                      <tr v-for="(harga, index) in hargaaxis" :key='index'>
                          <td class="text-center align-middle"><div class="avatar">
                              <img class="img-avatar" src="@/assets/img/provider/xlaxis-logo.png" width="60px">
                          </div></td>
                          <td class="align-middle">
        ...
        <div class="modal-dialog modal-primary" role="document">
          <div class="modal-content">
            <div class="modal-header bg-primary text-white">
              <h6 class="modal-title">Edit Nominal {{activerate.provider}}</h6>
            ...
            <div class="modal-body" v-if="!saving">
              <label class="text-muted" for="min">Minimal</label>
                <input class="form-control form-control-lg" type="number" name="min" id="min" v-model='activerate.min'><br>
              <label class="text-muted" for="min">Maksimal</label>
                <input class="form-control form-control-lg" type="number" name="max" id="max" v-model='activerate.max'><br>
              <label class="text-muted" for="min">Rate</label>
                <input class="form-control form-control-lg" type="number" name="rate" id="rate" v-model='activerate.rate'><br>
            </div>
            <div class="modal-body justify-content-center d-flex py-5" v-if="saving">
              <div class="mt-3">
                <orbit-spinner
                  :animation-duration="1200"
                  :size="55"
                  ...
                  style="margin:auto"
                />
                <p class="font-italic font-weight-bold small mt-4">Sedang menyimpan...</p>
              </div>
            </div>
            <div class="modal-footer" v-if="!saving">
              <button class="btn btn-outline-secondary" type="button" @click="activ = false">Batal</button>
    ...
    <transition name="fade">
      <div class="modal fade shadow show d-block" v-if="hapus" role="dialog">
        ...
        <div class="modal-content">
          ...
          <div class="modal-body justify-content-center d-flex py-5" v-if="deleting">
            <div class="mt-3">
              <orbit-spinner
                :animation-duration="1200"
                :size="55"
                ...
                style="margin:auto"
              />
              <p class="font-italic font-weight-bold small mt-4">Sedang menghapus...</p>
            </div>
          </div>
          <div class="modal-footer" v-if="!deleting">
            <button class="btn btn-outline-secondary" type="button" @click="hapus = false">Batal</button>
            <button class="btn btn-danger" type="submit" @click="rateHapus()">Hapus</button>
          </div>
        </div>
        <!-- /.modal-content-->
        </div>
...
export default {
  ...
  methods: {
    ...
    saveEdit(){
      ...
      this.saving = true
      var vm = this
      var data = db.collection('rateconvert').where('id', '==' , this.activerate.id)
        data.get().then(function(qSnap){
          qSnap.forEach(function(doc){
            console.log(doc.id)
            db.collection('rateconvert').doc(doc.id).update({
    ...
    rateHapus(){
      var vm = this
      this.deleting = true
      var data = db.collection('harga').where('min', '==', this.activerate.min).where('max', '==', this.activerate.max).limit(1)
        data.get().then(function(snap){
          snap.forEach(function(doc){
            db.collection('harga').doc(doc.id).delete().then(function(){
  ...
  computed: {
    hargatsel: function() {
      return this.hargas.filter(function(hrg){
        return hrg.provider == 'telkomsel'
      })
    },
    hargaxl: function() {
      return this.hargas.filter(function(hrg){
        return hrg.provider == 'xl'
      })
    },
    hargaaxis: function() {
      return this.hargas.filter(function(hrg){
        return hrg.provider == 'axis'
      })
    },
    hargaindosat: function() {
      return this.hargas.filter(function(hrg){
        return hrg.provider == 'indosat'
      })
    },
    hargathree: function() {
      return this.hargas.filter(function(hrg){
  ...
  firestore () {
    return {
      hargas: db.collection('hargajual').orderBy('nominal', 'asc')
    }
  }
}
</script>