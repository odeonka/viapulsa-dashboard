                  ...
                  <thead class="thead-light">
                    ...
                    <tbody>
                      <tr v-for="(rate, index) in ratevip" :key='index'>
                          <td class="text-center align-middle"><div class="avatar">
                              <img v-if="rate.provider.includes('telkomsel')" class="img-avatar" src="@/assets/img/provider/telkomsel-logo.png" width="60px">
                              <img v-if="rate.provider.includes('xlaxis')" class="img-avatar" src="@/assets/img/provider/xlaxis-logo.png" width="60px">
                          </div></td>
                          <td class="align-middle">
                              Rp. {{formatNominal(rate.min)}}
                          </td>
...
export default {
  ...
  methods: {
    ...
    rateHapus(){
      var vm = this
      this.deleting = true
      var data = db.collection('rateconvert').where('min', '==', this.activerate.min).where('max', '==', this.activerate.max).limit(1)
        data.get().then(function(snap){
          snap.forEach(function(doc){
            db.collection('rateconvert').doc(doc.id).delete().then(function(){
